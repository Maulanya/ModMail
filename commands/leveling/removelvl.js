...
if (!user) return message.channel.send({embed: {
                    color: 16734039,
                    description: "You must mention someone to remove level!"
                }})
    if (isNaN(args[1])) return message.channel.send({embed: {
                    color: 16734039,
                    description: "You must enter the amount of level to remove!"
                }})
                if(args[0] >= 20000)
                {
   ...
   return;
    ...
    db.delete(`${ao}level_${user.id}`, args[1])
    let remlvl = await db.fetch(`${ao}level_${user.id}`)
   
    let newxpEmbed = new Discord.MessageEmbed()
    .setColor("RANDOM")
    .setDescription(`<:pleux_success:887552715247452210> Delete ${args[1]} Levels from ${useram}, now level ${remlvl}`);
    message.channel.send(newxpEmbed)
    } else {
	message.channel.send({embed: {
                    color: 16734039,
                    description: "You don't have premission remove level!"
                }})
	}
}
exports.info = {
